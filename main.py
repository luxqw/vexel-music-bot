...
async def play(interaction: discord.Interaction, query: str):
    ...
    try:
        # ✅ ИСПРАВЛЕНИЕ #14: Асинхронный вызов yt-dlp
    ...
    # ✅ ИСПРАВЛЕНИЕ: Проверяем что info не None
    if not info:
        try:
    ...
    # ✅ ИСПРАВЛЕНИЕ: Дополнительная проверка entries
    if "entries" in info and info["entries"]:
        # ✅ ИСПРАВЛЕНИЕ #15: Ограничение размера плейлиста
        ...
        try:
            ...
            pass
    # ✅ ИСПРАВЛЕНИЕ: Улучшенная проверка одиночного трека
        ...
        # ✅ ИСПРАВЛЕНИЕ: Обработка случая с неполными данными
        try:
            await interaction.edit_original_response(
...
async def play_next(vc, guild_id):
    ...
    try:
        # ✅ ИСПРАВЛЕНИЕ #14: Асинхронное получение прямой ссылки на аудио
